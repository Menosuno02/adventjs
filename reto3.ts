...
function organizeInventory(inventory: Inventory): object {
  ...
  for (const toy of inventory) {
    ...
    if (!(toy.category in organizedInventory))
      organizedInventory[toy.category] = {};

    // Si el juguete no existe, lo creamos
    if (!(toy.name in organizedInventory[toy.category]))
      organizedInventory[toy.category][toy.name] = 0;
